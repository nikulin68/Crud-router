...
class ViewNotePage extends React.Component {
  ...
  componentDidMount() {
    this.props
      .get(`notes/?id=${this.props.params.id}`)
      .then((data) => {
        this.setState({ ...data });
      })
  ...
  render() {
    const { id, text, created } = this.state;

    const { onDeleteClick: handleDeleteClick } = this.props;
    ...
    return (
      <div className="Page">{!this.state.editingMode ? note : editing}</div>
    );
  }
}
